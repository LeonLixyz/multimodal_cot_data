...
def format_multimodal_trace(input_dir, prompt_file, output_folder):
    """
    ...
    Args:
        ...
        output_folder: Folder to save formatted messages and raw files
    """
    # Load prompt
    ...
    # Copy input directory to raw_files in output folder
    raw_files_dir = os.path.join(output_folder, "raw_files")
    if os.path.exists(raw_files_dir):
        shutil.rmtree(raw_files_dir)
    shutil.copytree(input_dir, raw_files_dir)
    print(f"Copied raw files to {raw_files_dir}")
    
    # Save to output file
    output_file = os.path.join(output_folder, "api_input_message.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_folder", required=True, help="Folder to save formatted messages and raw files")
    
    args = parser.parse_args()
    
    format_multimodal_trace(args.input_dir, args.prompt_file, args.output_folder)