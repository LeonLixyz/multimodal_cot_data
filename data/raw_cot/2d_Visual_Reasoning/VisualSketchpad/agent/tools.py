...
# Try to connect to Gradio servers with authentication
try:
    auth = ("admin", "admin")
    # Set of Marks
    som_client = Client(SOM_ADDRESS, auth=auth)
    # Grounding DINO
    gd_client = Client(GROUNDING_DINO_ADDRESS, auth=auth)
    # Depth Anything
    depth_client = Client(DEPTH_ANYTHING_ADDRESS, auth=auth)
    USING_REMOTE_SERVICES = True
except Exception as e:
    print(f"Warning: Couldn't connect to vision experts: {e}")
    print("Falling back to local implementations")
    USING_REMOTE_SERVICES = False
...
def depth(image):
    ...
    with tempfile.NamedTemporaryFile(delete=True) as tmp_file:
        image.save(tmp_file.name, 'JPEG')
        image = tmp_file.name
        outputs = depth_client.predict(file(image))
...
def find_parallel_intersection(A, B, C):
    ...
    norm = np.linalg.norm(direction_AE)
    if norm > 0:
        direction_AE_normalized = direction_AE / norm
    else:
        direction_AE_normalized = direction_AE
    
    # Extend by the length of BC
...
def find_perpendicular_intersection(A, B, C):
    ...
    # Calculate perpendicular from A to BC
    BC_length_squared = np.sum(BC**2)
    
    if BC_length_squared == 0:
    ...
    # Normalize to get the projection coefficient
    t = dot_product / BC_length_squared
